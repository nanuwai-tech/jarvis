...
This is intentionally simple -- a single local JSON file -- so the
assistant tools have somewhere durable to read and write without
requiring a database or any external account. Swap this module out for
a real database, or for calls to Google Calendar / Gmail / a CRM, once
you outgrow it; the function_tool wrappers in assistant_tools.py won't
...
def list_events(on_date: Optional[str] = None) -> list:
    events = _load()["events"]
    if on_date:
        events = [e for e in events if e["start"].startswith(on_date)]
    return sorted(events, key=lambda e: e["start"])


def next_event() -> Optional[dict]:
    now = datetime.now().isoformat(timespec="minutes")
    upcoming = [e for e in list_events() if e["start"] >= now]
...
def find_contact(name: str) -> Optional[dict]:
    ...
    matches = [c for c in data["contacts"] if name_l in c["name"].lower()]
...
def search_notes(query: str) -> list:
    ...
    return [
        ...
        if q in n["content"].lower() or (n["tag"] and q in n["tag"].lower())
    ]
